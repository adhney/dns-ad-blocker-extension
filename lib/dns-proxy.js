class DNSProxy {
  ...
  async initialize() {
    ...
    console.log('Loaded settings:', { enabled: this.enabled, blockListCount: this.blockLists.length });
    ...
    try {
      ...
      if (rules.length > 0) {
        ...
        for (let i = 0; i < rules.length; i += batchSize) {
          ...
          console.log(`Added batch ${i/batchSize + 1}/${Math.ceil(rules.length/batchSize)}`);
        }
      }
